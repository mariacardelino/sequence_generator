...
# Observer for generating sequence list ######################################################################################################################
  observeEvent(input$write_list, {
    ...
    tryCatch({
      ...
      script_files <- c("run_parameters.R", "c18_lines.R", "hilic_lines.R")
      missing_files <- character(0)
      
      for(file in script_files) {
        file_path <- file.path(parent_wd, file)
        if(!file.exists(file_path)) {
          missing_files <- c(missing_files, file)
          debug_log(paste("WARNING: Required file not found:", file_path))
        } else {
          file_size <- file.info(file_path)$size
          debug_log(paste("19. Found file:", file, "- Size:", file_size, "bytes"))
        }
      }
    ...
    sequence_filepath <- file.path(parent_wd, filename_value)
    ...
    tryCatch({
      ...
      debug_log(paste("23. Output directory:", output_dir))
    ...
    withProgress(message = 'Generating sequence list', value = 0, {
      ...
      tryCatch({
        ...
        # Try sourcing the first script
        debug_log("29. Attempting to source run_parameters.R...")
        incProgress(0.2, detail = "Loading parameters...")
        
        source_result <- try({
          source(file.path(parent_wd, "run_parameters.R"))
        }, silent = FALSE)
        
        if(inherits(source_result, "try-error")) {
          debug_log(paste("ERROR in run_parameters.R:", geterrmessage()))
          stop(paste("Error in run_parameters.R:", geterrmessage()))
        }
        
        debug_log("30. Successfully loaded run_parameters.R")
        
        # Proceed to machine-specific script
        incProgress(0.5, detail = paste("Processing", machine_value, "sequence..."))
        
        if(machine_value == "C18") {
          debug_log("31. Attempting to source c18_lines.R...")
          
          source_result <- try({
            source(file.path(parent_wd, "c18_lines.R"))
          }, silent = FALSE)
          
          if(inherits(source_result, "try-error")) {
            debug_log(paste("ERROR in c18_lines.R:", geterrmessage()))
            stop(paste("Error in c18_lines.R:", geterrmessage()))
          }
          
          debug_log("32. Successfully executed c18_lines.R")
        } else {
          debug_log("31. Attempting to source hilic_lines.R...")
          
          source_result <- try({
            source(file.path(parent_wd, "hilic_lines.R"))
          }, silent = FALSE)
          
          if(inherits(source_result, "try-error")) {
            debug_log(paste("ERROR in hilic_lines.R:", geterrmessage()))
            stop(paste("Error in hilic_lines.R:", geterrmessage()))
          }
          
          debug_log("32. Successfully executed hilic_lines.R")
        }
        ...
        if(file.exists(sequence_filepath)) {
          debug_log(paste("33. SUCCESS: File created at", sequence_filepath))
          debug_log(paste("34. File size:", file.info(sequence_filepath)$size, "bytes"))
        ...
        debug_log("35. Cleaning up global variables...")
      ...
      # Complete progress
      debug_log("36. Updating progress indicator to complete")
      incProgress(1, detail = "Complete!")
    })
    
    debug_log("37. Sequence generation process complete")
