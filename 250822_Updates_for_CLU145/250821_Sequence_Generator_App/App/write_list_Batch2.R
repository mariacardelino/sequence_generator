...
write_sequence <- function(params) {
  ...
  # pfas_ms2_positions <- c("C3", "E17", "G19", "C10", "E4", "M12") 
    # contained in CLU0145 Batch 2: C3 ,E17, G19. Missing last 3
  # nonpfas_ms2_positions <- c("E11", "G17", "C16", "I14", "M8", "O20")
    # contained in CLU0145 Batch 2: E11, G17 only. Missing last 4
  
  ## MODIFY ####
  
  ## CLU0145 MODIFIED TO HAVE 6 POSITIONS EACH IN SMALLLER BATCH ******
  pfas_ms2_positions <- c("C3", "E17", "G19", "A7", "G2" )
  nonpfas_ms2_positions <- c("E11", "G17", "O3", "E9", "K13", "A4")
  ...
  modify <- function(line, run_order, msms_type = NULL) {
    ...
    # MANUALLY CHANGE THIS TO GO AFTER THE CAL CURVE
    change_point <- 100
    
    # Choose the appropriate l2 value 
    if (run_order <= change_point) {
      l2_value <- rack1
    } else {
      l2_value <- rack2
    }
  ...
  ### MANUALLY WRITING FOR CLU0124 BATCH 2: 80 samples in rack 1, 10 samples in rack 2. just do one chunk.
  ...
  # Pool 1 (6/10)
  # Pool 2
  # 10 study samples - rack2
 
  # COULD REMOVE 3 pool - only need 7 each!!!!
  ...
  for (i in 1:batch_size) { #CLU0145: 1-20
  ...
  for (i in ((batch_size+1):(batch_size*2))) { #CLU0145: 21:40
  ...
  for (i in ((2*batch_size+1):(batch_size*3))) { #CLU0145: 41-60
  ...
  for (i in ((3*batch_size+1):(batch_size*4))) { #CLU0145: 61-80
  ...
  ### EDIT FOR CLU0145 BATCH 2!!! RACK 2 - 10 samples
  for (i in (81:90)) { #CLU0145: HARD CODE 81-90!!!! 
  ...
  #1 MDL - the last cal curve qaqc sample
