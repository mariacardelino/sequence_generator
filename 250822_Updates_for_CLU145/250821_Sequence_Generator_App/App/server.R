...
## TO-DO #### ## TESTING FOR CLU0145

# Translate this new server, ui, and write_list to the github repo that has a new run order. 

# big changes are 
 #' the study_samples/ordered_study_samples not overwritten anymore
 #' the method button choices now (remote/not remote)
 #' the UI for this ^
 #' working on a more robust way to distinguish between the first and second rack (for ID purposes)
...
# The automatic order detection in sample inventory is wrong 
  ...
  # SOLUTION FOR NOW IS TO ONLY DO MANUAL ORDER SELECTION. We will see if it works and goes to row-wise. 
  # Manual check works. 


## CLU0145 - change below AND AT LINE 1416 (Batch 1 or Batch 2.) ----------
# source("write_list_Batch1.R") 
source("write_list_Batch2.R")
# source("write_list.R")
...
server <- function(input, output, session) {
  ...
  order_pattern <- reactiveVal("byrow") # DEFAULT BY ROW
  ...
  observeEvent(input$match_samples, {
    ...
    # inventory_loc <- c() # removed 8/21 not useful
    ...
    for (i in 1:nrow(current_data)) {
      ...
      if (sample_type == "QAQC") {
        ...
        if (length(match_row) > 0) {
          ...
          # inventory_loc <- c(inventory_loc, match_row) #REMOVED 8/21 not useful
    ...
    ###### REMOVED SECTION: AUTOMATIC ORDER DETECTION (NOT WORKING 8/21) #############
    ...
    # Manually setting order pattern: watch for changes to radio buttons
    ...
    output$order_by_ui <- renderUI({
      radioButtons(
        ...
        choices = c("Column-wise ordering (A1, B1, C1...)" = "Col_order_by_plate", 
                    "Row-wise ordering (A1, A2, A3 ...)" = "Row_order_by_plate"),
  ...
  observeEvent(input$save_run_info, {
    ...
    if (machine == "C18") {
      values$final_paths$suggested_method_folder <- "C:/Xcalibur/methods/Aria_C18_Methods" # changed 8/21
      name <- 'RALPH'
    } else if (machine == "HILIC") {
      values$final_paths$suggested_method_folder <- "C:/Xcalibur/methods/Aria Methods" # changed 8/21
      name <- 'NANCY'
    }
    ...
    updateTextInput(session, "method_folder", value = values$final_paths$suggested_method_folder) # 8/22 trying for switching between c18 and hilic
  ...
  observeEvent(input$confirm_paths, {
    values$final_paths$project_path <- input$project_path
    values$final_paths$method_folder <- input$method_folder
    
    if (is.null(values$final_paths$remote_method)) {
      values$final_paths$remote_method <- TRUE
    }
    
    values$final_paths$output_path <- input$output_path
  ...
  # If first button is clicked (User is on instrument computer) 
  observeEvent(input$btn_local, {
    values$final_paths$remote_method = FALSE
    updateTextInput(session, "method_folder", value = values$final_paths$suggested_method_folder) 
  })
  
  # If second button is clicked (User is on a different computer)
  observeEvent(input$btn_remote, {
    values$final_paths$remote_method = TRUE
    updateTextInput(session, "method_folder", value = values$final_paths$suggested_method_folder)
  })
  ...
  observeEvent(input$write_list, {
    ...
    remote_method <- values$final_paths$remote_method # is the user on a remote machine?
    ...
    tryCatch({
      ...
      script_file <-"write_list_Batch2.R"
      #script_file <-"write_list_Batch1.R"
    ...
    withProgress(message = 'Generating sequence list', value = 0, {
      ...
      tryCatch({
        ...
        params <- list(
          ...
          remote_method = remote_method, # 8/22
